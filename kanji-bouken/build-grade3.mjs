/* 漢字の冒険：小3ストロークデータ・ビルドスクリプト（KanjiVG版）
 * 小3配当漢字 200字（習〜湯・授業の順番どおり）の SVG を取得し
 * grade3-strokes.js として同梱する（実行時 通信なし）。
 *
 * 出典: KanjiVG (c) Ulrich Apel / CC BY-SA 3.0  http://kanjivg.tagaini.net
 * 使い方:  node kanji-bouken/build-grade3.mjs
 */
...
/* 小3配当漢字 200字（授業の順番どおり・178番目は「院」に修正済） */
const KANJI = [
  '習','詩','動','商','物','族','開','登','事','面',
  '所','葉','実','号','館','意','酒','問','味','使',
  '深','漢','温','平','題','調','章','由','発','県',
...
const out =
`/* 自動生成：build-grade3.mjs（編集しないでください）
 * 出典: KanjiVG (c) Ulrich Apel / CC BY-SA 3.0  http://kanjivg.tagaini.net
 * 座標系: 109×109（Y下向き・反転不要）
 * stroke=中心線パスd / median=平坦化サンプル座標
 * 漢字 ${ok}字 / 生成日時 ${new Date().toISOString()}
 */
const GRADE3_STROKES = ${JSON.stringify(data)};
if (typeof module !== 'undefined') module.exports = GRADE3_STROKES;
`;

await writeFile(join(__dirname, 'grade3-strokes.js'), out, 'utf8');
console.log(`\n書き出し完了: grade3-strokes.js (${ok}/${KANJI.length}字)`);
