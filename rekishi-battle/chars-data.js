...
/* ===== 「2」「3」シリーズ（2＝2026-08-08 / 3＝2026-08-09。どちらもユーザー要望）=========
   ...
   ★2026-09-23に 「3」の あとへ **「4」（超激ムズ・やりくり）** を足した＝17×4＝**68ステージ**（下の HARD4）。
...
/* ★★★「4」＝超激ムズ・やりくり（2026-09-23・ユーザー指示
     「歴史バトルをクリアする子が増えてきたので ステージ4を追加。激ムズにして、
      **うまく やりくりしないと クリアできない**感じで」）。
   ★★4の しかけは **おかねが たまるのが おそい**こと（`incomeMul`）。敵の強さは 3と同じ帯で、
     そのうえで 1秒に たまる おかねを しぼってある。
     ＝ **人を出す／しろの ちからを 使う／ためる** の どれに 使うかを 1回ずつ 考えないと 足りない。
     3では「おかねは 調整レバーに 向かない」と 書いたが、それは **同時に10体 出る 3の波に
     おかねだけ しぼった**から。4は **いちどに 出る ざこを 少なく・1体を かたく**して
     「人数で おしきる」より「むだづかいしない」が 勝ちすじに なるように 形ごと 変えてある。
   ★★数字は ぜんぶ **シミュレーションで 決めた 叩き台**（HANDOFF「「4」＝超激ムズ」に 手順と 結果）。
     ねらいの線: **Lv50の いちばん強いデッキ＋かしこい つかいかた で やっと勝てる**／
       Lv30（SRになりたて）や、おかねを すぐ使いきる あそびかたでは 勝てない。
   ★id は もとの id ＋ "d"（st1→st1d）。`series: 4`。そのほかは 2・3と まったく同じ作り。 */
var HARD4 = {
  mhp:   9.0,   /* ざこ敵のHP（3より かたい。そのかわり 数を へらす＝下の maxMul） */
  matk:  4.8,   /* ざこ敵のこうげき */
  bhp:   6.5,   /* ボスのHP */
  batk:  4.2,   /* ボスのこうげき */
  ehp:   2.3,   /* 敵の城（3と同じ。★上げない＝3と同じ理由） */
  every: 0.62,  /* 波と波の あいだ */
  maxUp: 1,     /* 同時に出るざこ +1 */
  maxMul: 1.0,  /* ★3の 1.44倍は かけない（数より 1体の重さで せめる） */
  mspd:  1.2,   /* ざこの あるく はやさ */
  need:  2.3,   /* つくる型の しごとの量 */
  sec:   1.2,   /* つくる型の 制限時間 */
  base:  1.15,  /* みかたの城のHP */
  income: 0,    /* 1秒にたまるおかね（+0。3は +6） */
  incomeMul: 0.7 /* ★4の しかけ。そのうえで 0.7倍（st1 10→7／st17 15→11 びょう） */
};
(function () {
  ...
  function makeSeries(suffix, num, mark, tail, H) {
    return origin.map(function (st) {
      ...
      /* ★incomeMul は 4シリーズだけの もの（書いていない 2・3は これまでどおり 足すだけ） */
      s.income = Math.round((st.income + H.income) * (H.incomeMul || 1));
  ...
  var made = makeSeries("b", 2, "２", "　【2】もっと つよい てきが せめてくる！", HARD2)
        ...
        .concat(makeSeries("d", 4, "４", "　【4】超《ちょう》激《げき》ムズ！ おかねが たまるのが おそい。やりくりが カギ！", HARD4));
