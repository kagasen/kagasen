// 宿題提出ポスト — Service Worker
// オフライン動作とPWAインストールのためのキャッシュ。
// アセットを更新したら CACHE のバージョン番号を上げる(v1->v2...)。
const CACHE = 'shukudai-cache-v2';
const ASSETS = [
  './',
  './backup-kit.js?v=2',
  './barcode-detector.min.js',
  './index.html',
  './qrcode.min.js',
  './tailwind.css',
  './manifest.json',
...
self.addEventListener('activate', (e) => {
  e.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(keys.filter((k) => k.startsWith('shukudai-cache-') && k !== CACHE).map((k) => caches.delete(k)))
