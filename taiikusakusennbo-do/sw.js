// 体育作戦ボード — Service Worker
// オフライン動作とPWAインストールのためのキャッシュ。
// アセットを更新したら CACHE のバージョン番号を上げる(v1->v2...)。
const CACHE = 'taiikusakusennbo-do-cache-v1';
const ASSETS = [
  './',
  './index.html',
  './tailwind.css',
...
self.addEventListener('activate', (e) => {
  e.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(keys.filter((k) => k.startsWith('taiikusakusennbo-do-cache-') && k !== CACHE).map((k) => caches.delete(k)))
