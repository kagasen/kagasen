...
const CACHE = 'vision-training-cache-v1';
const ASSETS = [
  './',
  './count-items/index.html',
  './find-diff/index.html',
  './find-same/index.html',
  './fonts.css',
...
self.addEventListener('activate', (e) => {
  e.waitUntil(
    caches.keys().then((keys) =>
      Promise.all(keys.filter((k) => k.startsWith('vision-training-cache-') && k !== CACHE).map((k) => caches.delete(k)))
